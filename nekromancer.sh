...
oldPackageName="io.intrepid.skotlinton"
oldApplicationName="skotlinton"
oldApplicationCapitalizedName="Skotlinton"
oldDirectoryName="io/intrepid/skotlinton"
...
OPTIONAL:
  c      Start with a clean history. By default the project created by this
         script will have all the history of the Skotlinton project included.
...
if [ "$newPackageName" != "$oldPackageName" ]; then
    ...
    find . -type f \( ! -iname "*.png" \) -exec sed -i '' "s/$oldApplicationName/$newApplicationName/g" {} \;
...
if [ "$autoCommit" = true ] ; then
  git commit -m "Initial import from Skotlinton"
fi
